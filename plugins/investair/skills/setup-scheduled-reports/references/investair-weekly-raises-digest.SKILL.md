...
- Headline: `summary.deal_count` raises in the window, and note if `summary.truncated` is true
- A short table from `ranked`: Ticker, Company, Discount (if available), DAP (if available). Do not cite AUD proceeds or issue price
